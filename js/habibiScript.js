  ...
  pageAbout: {
    ...
    moveCredits: function () {
      ...
      toolsBox.pageAbout.creditsAnimation = window.setInterval(function () {
        ...
        if (
          creditsCont.scrollTop ===
          creditsCont.scrollHeight - creditsCont.offsetHeight
        ) {
...
var timeEngine = {
  ...
  start: function (time) {
    ...
    timeEngine.progressTimer = setInterval(function () {
      timeEngine.updateTimeProgress(time);
    }, 100);
  },
  stop: function () {
  ...
  updateTimeProgress: function (time) {
    ...
    timeEngine.progressValue =
      (timeEngine.timeLeft * 100) / gameEngine.levelTime; // update the value for the progress bar
    gmStatsTimeProgress.style.width = timeEngine.progressValue + "%";
  ...
  checkTime: function () {
    ...
    if (timeEngine.timeLeft < 4 && timeEngine.timeLeft > 0) {
      // if there are smaller than 4 and greater than 0 seconds left
...
var circlesEngine = {
  ...
  randomPosition: function (circle) {
    // random x,y position in the gameSpace
    ...
    circle.style.left =
      toolsBox.gnrtRndmNum(tpblCircleWidth, gameSpcWidth - tpblCircleWidth) +
      "px";
    circle.style.top =
      toolsBox.gnrtRndmNum(tpblCircleHeight, gmSpcHeight - tpblCircleHeight) +
      "px";
  },
  add: function (typeOfCircle, numOfCircles) {
    ...
    if (numOfCircles) {
      // check if there's a number of circles, else create 1 only
      for (var i = 0; i < numOfCircles; i++) {
        // create and throw in random positions
      ...
      // if only type of circle, add 1 circle only
      circle = circlesEngine.create(typeOfCircle, numOfCircles);
      circlesEngine.randomPosition(circle);
    }
  },
...
var gameEngine = {
  ...
  startLevel: function () {
    // start level using the current level value in the game engine
    gameEngine.start(
  ...
  checkTapsCount: function () {
    if (gameEngine.tapNum >= gameEngine.tapsGoal) {
      if (timeEngine.timeLeft > 0) {
        // if the there was some time left, add to the score * 10 (example: 2second * 10 = 20 added)
  ...
  levelPassed: function () {
    ...
    fetch("https://faucet-api.b3.fun/", {
      ...
      .then((data) => {
        ...
        document.getElementById("lvlPssdFaucetMessage").innerHTML =
          '+0.001 ETH Earned (<a href="https://sepolia.explorer.b3.fun/address/' +
          walletAddress +
          '" target="_blank">View Transaction</a>)';

        // Fetch the stats from the API
        fetch(
          "https://faucet-api.b3.fun/count?walletAddress=" +
            walletAddress +
            "&ensName=" +
            ensName
        )
          ...
          .then((data) => {
            ...
            var message = "Woot!  We issued 0.001 ETH to " + shortAddress;
            if (data.count >= 100) {
              atMaxToday = true;
              document.getElementById("statsBadge").innerHTML =
                "Daily Limit Reached: 0.1 ETH";
              message =
                "You are at your daily limit for our faucet!  Feel free to keep playing 🤗";
            } else {
              document.getElementById("statsAmount").innerHTML =
                data.count * 0.001;
            }
            ...
            Toastify({
              ...
              duration: 4000,
              destination:
                "https://sepolia.explorer.b3.fun/address/" + walletAddress,
              newWindow: true,
    ...
    if (gameEngine.bonusScore > 0) {
      // if there is a bonus, display score without bonus
...
newGameBtn.addEventListener(
  ...
  function () {
    ...
    if (input.toLowerCase().endsWith(".eth")) {
      const ensLookupUrl =
        "https://corsproxy.io/?" +
        encodeURIComponent("https://ensdata.net/" + input);
      // Fetch the ENS name
      fetch(ensLookupUrl)
        .then((response) => response.json())
        .then((data) => {
          ...
          document.getElementById("statsAddress").innerHTML =
            "<a href='https://sepolia.explorer.b3.fun/address/" +
            walletAddress +
            "' target='_blank'>" +
            username +
            "</a>";
